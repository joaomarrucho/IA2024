...
class Connect4AI:
    ...
    def _search(self, board, depth, alpha, beta, maximizing_player):
        winner = Connect4().check_winner()  # Corrigindo a chamada do método check_winner
        if winner == 'X':
            return 512 - depth
        elif winner == 'O':
            return -512 + depth
...
class Connect4GUI:
    ...
    def ai_make_move(self):
        if self.mode == "Humano vs IA" and self.algorithm == "A*":
            ...
            print("AI está a tentar jogar na coluna", column)  # Adicionando instrução de impressão
            if self.game.make_move(column):
                winner = self.game.check_winner()
                self.draw_board()
                if winner:
                    ...
                    if self.current_player_label is not None:  # Verifica se o rótulo do jogador atual não é None
                        self.current_player_label.config(text=f'Jogador atual: {self.game.current_player}')
